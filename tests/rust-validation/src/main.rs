...
///
/// D2: the TypeScript decoders report finer error codes where the reference
/// collapses everything raised inside the `ur` crate to `Error::UR(_)`
/// (bytewords failures during UR parsing are `Bytewords`, not `Decoder`);
/// `MultipartDecoder` also accepts single-part URs and lower-cases its input
/// the way `UR::from_ur_string` does.
///
...
fn expected_divergence(r: &serde_json::Value, got: &str, want: &str) -> Option<&'static str> {
    ...
    match k {
        "mpDecode" if r.get("parts").is_some() => Some("D2"),
