...
export function recipeName(r: Recipe): string {
  switch (r.k) {
    ...
    case "mpDecode":
      if ("parts" in r) return `mpDecode explicit ${r.parts.length} parts`;
...
export function materialize(api: VectorApi, r: Recipe): Outcome {
  try {
    switch (r.k) {
      ...
      case "mpEncode":
        return api.mpEncode(r.type, toBytes(r.cbor), r.maxLen, r.parts).join(" ");
      case "mpDecode": {
        const parts =
          "parts" in r
            ? r.parts
            : orderParts(
                api.mpEncode(r.from.type, toBytes(r.from.cbor), r.from.maxLen, r.from.parts),
                r.order,
                r.drop,
              );
 ...
 */
export function redesignedAdapterFor(m: any, bw: any, dcbor: any): VectorApi {
