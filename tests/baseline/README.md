...
commit `899897dbba1fbca58bb212600b3266f14a062a15`, the first implementation's wire-format
reference. The sibling `@blockchaincommons/crypto` is INLINED from its own frozen baseline
bundle, and that implementation's `@blockchaincommons/dcbor-compat` is INLINED directly, so
this bundle keeps the behaviour of its dependencies at that commit after they change.
