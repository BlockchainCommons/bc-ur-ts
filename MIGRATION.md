...
Messages are unchanged (`invalid UR scheme`, `expected UR type X, but found
Y`, `Bytewords error (invalid checksum)`, …). `Result<T>` and `isError` are
gone. Argument-domain errors (`maxFragmentLength < 1`, an empty message, a
short identifier that is not 4 bytes) are `RangeError`.

```ts
try {
  UR.parse(s);
} catch (e) {
  if (URError.isURError(e) && e.code === "UnexpectedType") console.log(e.details.expected);
}
```
